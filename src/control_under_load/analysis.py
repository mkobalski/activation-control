...
def _weighted_sd(values: np.ndarray, weights: np.ndarray) -> float:
    # This is the d' DENOMINATOR (Appendix B: the across-item SD of the baseline
    # matched-minus-mean-wrong contrast), so it must not be optimistic. Two guards:
    # a bootstrap draw can resample down to a degenerate cell, and an SD from one or
    # two effective items is noise that would inflate every d' built on it. Returning
    # NaN drops the cell rather than propagating a spuriously small denominator.
    keep = np.isfinite(values) & np.isfinite(weights) & (weights > 0)
    total = float(np.sum(weights[keep]))
    if total <= 1 or np.count_nonzero(weights[keep]) < 3:
        return float("nan")
    mean = np.sum(values[keep] * weights[keep]) / total
    # ddof=1 on the WEIGHT total, not the element count: under a bootstrap the weights
    # are integer multiplicities, so total is the effective sample size.
...
def _weighted_slope(y: np.ndarray, x: np.ndarray, weights: np.ndarray) -> float:
    # The headline statistic: OLS slope of the gap on the ORDERED recipe index (x is
    # the native bin index 0..4, never a composite difficulty score). Weights carry
    # the bootstrap multiplicities, so resampling an item twice counts it twice here
    # rather than duplicating rows. A draw that collapses x to one distinct bin has
...
def _metric(value: float, replicas: np.ndarray, n: int) -> dict[str, float | int]:
    # Percentile interval, not normal-approximate: `value` is the point estimate from
    # the observed data and the CI is the 2.5/97.5 percentiles of the replicate
    # distribution, so the two are computed independently and the interval need not be
    # symmetric about the estimate. Which axes each replicate resamples is the caller's
    # choice -- headline draws resample items AND concepts and reselect the pooled peak
    # layer, per-concept draws resample items only at the observed peak.
    return {
        "value": float(value),
