...
``experiments/main/config.yaml``). This module mirrors that YAML structure as a
...
def _build_config(raw: Dict[str, Any]) -> ExperimentConfig:
    ...
    #
    # Mirrors run_experiment.LT_ONLY_SETS, which decides the `_lt` run-dir tag from
    # the same idea: a set that is standalone here must be LT-only there. Asserted in
    # tests/test_lt_sets_in_sync.py so the two cannot drift silently.
...
def parse_cli_args() -> argparse.Namespace:
    ...
    p.add_argument("--config", type=str, default="experiments/main/config.yaml")
