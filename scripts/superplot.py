...
controllability scalar S, across the figure_scripts/roster.py panel (models with
no SCORES_<model>.json on disk are skipped).
...
so it is fast and deterministic. The post-run orchestrator regenerates it after
each run. Rendered output is gitignored
...
# The roster -- models, family order, display labels and release dates -- is
# single-sourced from figure_scripts/roster.py so this figure and the paper's
# other cross-model panels cannot disagree about who is in the panel, what a
# model is called, or where it sits.
sys.path.insert(0, str(PROJECT_ROOT / "figure_scripts"))
from roster import ROSTER as MODELS, FAMILY_ORDER, DETAIL_ALL as DETAIL, RELEASE  # noqa: E402
...
def load_bars(data_root, channel="proj"):
    ...
    for name, fam, size in MODELS:
        ...
        # unchanged. See README. Falls back to SCORES if absent.
...
def _layout(bars):
    ...
    same rule the paper's scalar figure (figure_scripts/scalar.py) uses, keyed on
