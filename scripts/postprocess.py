...
Runs the standalone steps in sequence -- each remains its own editable script:
    ...
    figdata.py            -> FIGDATA_<model>.json (focal-panel figure inputs)
...
def main():
    ...
    fd = [SCRIPTS / "figdata.py", "--main-run", main_run, "--model", model,
          "--data-root", dr, "--json", dr / f"FIGDATA_{model}.json"]
    if lt_run is not None:
        fd += ["--lt-run", lt_run]
    _step(fd, "figdata -> FIGDATA")
