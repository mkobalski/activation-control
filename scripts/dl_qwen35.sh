...
# Resumable; logs progress. Intended to run in the background before a run.
set -euo pipefail
set -a; [ -n "${AC_ENV_FILE:-}" ] && source "$AC_ENV_FILE"; [ -f .env ] && source .env; set +a
echo "START $(date -u +%H:%M:%S) HF_HOME=$HF_HOME"
"${AC_PYTHON:-python3}" - <<'PY'
import os, time
