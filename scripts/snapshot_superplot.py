...
(RUNBOOK_olmo_snapshots_1xH100.md) is a within-model training-time curve, not
part of the retained model panel; superplot's explicit MODELS list excludes the
...
Lives in the pod-local checkout (/local/ac-gpt20) per the shared-volume rules;
the rendered figure goes into the shared results/ store:
    results/olmo3_7b_snapshot_comparison.png / .pdf
"""
import argparse
import json
...
def render(pts, out_stem, channel):
    ...
    for ax, (key, ylab) in zip(axes, ROWS):
        ...
        # (same pathology that excludes ministral3_3b from the panel — superplot.py
        # note). Clip the axis to the data + well-behaved CIs so one exploding
