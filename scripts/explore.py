...
Rendered into a run's own folder (results/raw/<run>/figures/) by
scripts/postprocess.py after a run scores. Projection channel throughout, except
...
Figures (main runs only; LT runs get none):
  ...
  channels_depth      engage & suppress d' vs depth, one panel per readout channel
...
Sensitivity figures use conventional d' throughout: positive means the readout
rose under the instruction, the same convention as the paper.

Usage:
  python scripts/explore.py --run-dir results/raw/<RUN>
  python scripts/explore.py --run-dir <RUN> --out <DIR> --only channels_depth,intensity_rank
"""
import argparse
import sys
...
# ======================================================================================
# 2. channels_depth — engage/suppress d' vs depth, one panel per readout channel
# ======================================================================================

def channels_depth(run_dir, out):
    """Engage and suppress sensitivity vs depth on all three channels, shared axis.

    Both curves are CONVENTIONAL d' = (instructed - baseline)/sigma, so positive
    means the readout ROSE under the instruction -- the same convention as the
    paper's per-channel depth figure. Suppression typically fails to drive the
    concept below baseline and mildly rebounds, so it sits at or above zero.
    """
    layers, vals, bases = sd.unit_layer_readouts(run_dir, [POS, NEG])
    pcts = fs.depth_pcts(layers, sd.run_n_layers(run_dir))
    channels = [("cos", "cosine  (direction)"), ("relnorm", "relative norm  (magnitude)"),
                ("proj", "projection")]
    fig, axes = plt.subplots(len(channels), 1, figsize=(7, 8.4), sharex=True, sharey=True)
    for ax, (ch, chlab) in zip(axes, channels):
        order = sorted(bases[ch])
        rng = np.random.default_rng(0)
        for cond, color, lab in ((POS, fs.ENGAGE_C, "think about"),
                                 (NEG, fs.SUPPRESS_C, "don't think about")):
            blocks, S = cs.per_concept_blocks(vals[(ch, cond)], bases[ch], order)
            if not blocks:
                continue
        ...
        ax.set_title(chlab, fontsize=10)
    axes[-1].set_xlabel("depth (%)")
    axes[0].legend(frameon=False)
    fig.suptitle("channels_depth — sensitivity vs depth, per readout channel\n"
                 "(positive = the readout rose under the instruction)", fontweight="bold")
    fig.tight_layout()
    fs.note(fig)
    return fs.save(fig, out)
...
def intensity_rank(run_dir, out):
    """Mean signed Spearman rho between instructed level and readout, vs depth.

    Uses the battery's own curve builder (compute_scores._profile_rank), the
    function whose output is stored as PROFILES `rank_lexical` / `rank_numeric`
    and plotted as Dial Rank in the paper's depth figure -- so the numeric curve
    here is the same statistic, on the same units, with the same band.
    """
    conds = [POS, LEX_HI] + list(RAMP)
    layers, vals, bases = sd.unit_layer_readouts(run_dir, conds)
    order = sorted(bases["proj"])
    n_L = len(layers)
    pcts = fs.depth_pcts(layers, sd.run_n_layers(run_dir))
    fig, ax = plt.subplots(figsize=(7, 4.4))
    for cs_conds, levels, color, lab in (
            ([POS, LEX_HI], [0, 1], fs.LEX_C, "think \u2192 intensely"),
            (list(RAMP), [1, 2, 3, 4], fs.NUM_C, "intensity 1 \u2192 4")):
        mean, lo, hi = cs._profile_rank(vals, "proj", cs_conds, levels, order, n_L, 2000)
...
def pos_coverage(run_dir, out):
    ...
    # Both panels are CONVENTIONAL d' (positive = the readout rose under the
    # instruction), matching the paper. Suppression rebounds rather than pushing
    # the concept below baseline, so its bars sit at or above zero.
    for ax, (cond, color, lab, mark) in zip(axes, ((POS, fs.ENGAGE_C, "think about", "weakest"),
                                                   (NEG, fs.SUPPRESS_C, "don't think about", "largest rebound"))):
        ...
        # Degenerate bootstrap tails: when a replicate's baseline sigma approaches
        # zero the interval explodes (1e11 and up on low-compliance models), which
        # is not a plotted range -- matplotlib's tick locator underflows on it.
        # Drop that bar's whisker, as superplot.py does for the same reason.
        ok = np.isfinite(lo) & np.isfinite(hi) & (np.abs(lo) < 1e3) & (np.abs(hi) < 1e3)
        if ok.any():
            ax.errorbar(x[ok], dp[ok], yerr=[(dp - lo)[ok], (hi - dp)[ok]], fmt="none",
                        ecolor="#333", elinewidth=0.8, capsize=2)
        if not ok.all():
            ax.text(0.99, 0.02, "some intervals degenerate; whisker omitted", transform=ax.transAxes,
                    ha="right", va="bottom", fontsize=6, color="#999")
        gi = int(np.nanargmin(dp) if cond == POS else np.nanargmax(dp))
        ax.annotate(mark, (x[gi], dp[gi]), textcoords="offset points", xytext=(0, 8),
