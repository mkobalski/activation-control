...
Method. The formal definition is Appendix A of the paper ("Formalization of the
Activation Controllability Benchmark"); the calibration constants and the reasons
behind them are in the D_REF and NOTES blocks below.
  ...
  1. Keep six measures (drop Token group = near-universal failure,
     ...
     unpaired conversion, redundant with dial_rank):
     engage, suppress, dial_rank, temporal_control, coverage, layer_targeting.
     Scored on the projection channel by default (--channels legacy keeps cos and
     relnorm as separate components instead). suppress and layer_targeting were
     folded back in on 2026-07-23
  ...
  2. Map each (measure, channel) score to a probability p in [0,1] where
     ...
     (--link, default 'linear'; see the D_REF block below for both forms):
       - d' measures (engage, suppress, temporal_control, coverage,
         layer_targeting):
             p = clip(0.5 + d' / (2 * D_REF[measure]), 0, 1)
  ...
  5. MEASURE-EQUAL weights: each of the (present) measures carries 1/N_measures;
     ...
     So under --channels legacy a two-channel measure gives 1/(2N) per channel
     and coverage, which has no relnorm channel, gives its full 1/N to cos.
     Under the default projection every measure has one channel, so each
     carries 1/N. Missing channels/measures are dropped and the remaining
...
bootstrap lives in scripts/scalar_ci.py, which emits SCALAR_CI_<model>.json;
this script reports the point estimate only.
...
#
# D_REF -- the score that means "essentially perfect control" on each axis, read on
# the projection channel.
#   engage, coverage, temporal_control: set just above that axis's ROSTER-MAX d'
#     (x1.02 margin), so the strongest model lands at p ~ 0.99 rather than pinned at
#     1.0 and the axis keeps discriminating across its full observed range.
#   suppress: anchored at d' ~ 3 = "reliably separable" given the axis's headroom.
#     Not lower: at 1.0 any suppress d' >= 1 saturates, and the lone below-baseline
#     outlier (Gemma 4 31B, d' ~ 1.45, wide CI) would then earn MORE credit than the
#     honest ~0.5 suppressors.
#   layer_targeting: anchored a priori at 5.0 on the STANDARDIZED-CONTRAST scale (the
#     Delta/sigma construction it shares with temporal control), deliberately NOT
#     derived from its roster max of ~0.05 -- calibrating a designed null to its own
#     maximum would spread p across [0.18, 0.99] and turn noise around zero into an
#     apparent capability. At 5.0 every model sits at p ~ 0.5: a uniform drag that
#     does not reorder models, and independent of temporal control's constant.
#   dial_resolution: retained for the diagnostic, which is computed and reported
#     outside S.
...
# ---- NOTES on this calibration (2026-08-10) -------------------------------------
...
#    measurement, so "six measures" spans five DISCRIMINATING axes. Kept in S, and
#    reported alongside as a designed null.
...
#    roster-max rule above exists to avoid. Measured on this roster --
...
def _to_prob(measure, cell, link="linear"):
    ...
    # This lets a measure that genuinely reaches its ceiling read as 1: on the current
    # roster that is Llama 3.1 8B, whose dial rank is exactly rho = 1. Effect on its
    # S is +2.6e-07; no other model moves.
...
def main():
    ...
    print("Excluded (diagnostics): dial_resolution (redundant with dial rank), "
          "onset_offset_error (coarse ↓-error), token_group (near-universal failure).")
