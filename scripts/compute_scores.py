#!/usr/bin/env python3
"""Compute the SCORES.md battery on all three readout channels, with uncertainty.
...
Channel conventions: instructions define a direction on every channel --
cosine: engage = toward the concept, suppress = away; relative norm:
engage = norm UP (add activation mass), suppress = norm DOWN; projection
(||r||*cos, the paper's channel) inherits both. Row 10 (coverage) is
cosine-derived (mixed signs on the norm channel).
...
def main():
    ap = argparse.ArgumentParser(description="Compute the SCORES.md battery from the stored run data.")
